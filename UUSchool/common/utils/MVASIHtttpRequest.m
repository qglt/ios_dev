...
- (void)requestForGetWithPramaStr:(NSString*)requestParamStr method:(NSString*)methodStr serverType:(int)serverType
{
    ...
    NSString *server = nil;
    if (serverType==1) {
#warning 配置服务器地址 server = ID_API_SERVER1;
        encodingNum=kCFStringEncodingUTF8;
    }else{

#warning 配置服务器地址 server = ID_API_SERVER4;
        encodingNum=kCFStringEncodingUTF8;
    }
...
//
    ...
    if(!responseString||[@"" isEqualToString:responseString]){
        ...
        NSDictionary * dict = @{@"returnMsg": @"   "};
        _listener(dict);
        [self stopRequest];
        return;
    }
    
    NSDictionary *json= [newString objectFromJSONString];
    LOG_GENERAL_INFO(@"[request responseString] ---->%@\n\nresponseString----->%@",[request responseString],responseString);
    LOG_GENERAL_INFO(@"new string--> %@,json  %@",newString, json.description);
...
- (void)requestFailed:(ASIHTTPRequest *)request{
    ...
    NSDictionary *dict = @{@"returnMsg": @"链接错误 "};
